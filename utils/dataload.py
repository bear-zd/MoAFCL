...
class ImageTextData(object):
    def __init__(self, root, client_name, preprocess, prompt="a picture of a"):
        client_data_path = osp.join(root, client_name)
...
class DomainDataset:
    ...
    def get_dataloader(self):
        ...
        for _, client_name in enumerate(os.listdir(self.root_dir)):
            data = ImageTextData(self.root_dir, client_name, self.preprocess)
            l = len(data)
            index = np.arange(l)
